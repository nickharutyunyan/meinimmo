...
export function deterministicAssessment(raw: string, source: string): Report {
  ...
  const energy = match(text, /(?:Energieeffizienzklasse|Effizienzklasse)\s*[:\-]?\s*([A-H][+]?)/i) || 'not stated'; const heating = match(text, /(?:Heizungsart|Heizung)\s*[:\-]?\s*([^,.]{1,40})/i) || 'not stated'; const address = match(text, /([A-ZÄÖÜ][\wäöüß.\- ]*(?:straße|str\.|allee|weg|platz|gasse)\s+\d{1,3}[a-z]?,?\s*\d{5}\s+[A-ZÄÖÜ][\wäöüß.\- ]+)/i) || 'Address not stated'; const location = listingLocation(raw, text, source); const titleLocation = street(address) || location;
