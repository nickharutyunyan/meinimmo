...
export async function POST(request: NextRequest) {
  ...
  const input = await request.json() as { username?: string; password?: string; name?: string; locale?: 'en' | 'de' };
  ...
  try {
    const user = await createCredentialsUser(input.username || '', input.password || '', input.name);
