...
export function GlossaryText({ children }: { children: string }) {
  const pieces = children.split(matcher);
  return <>{pieces.map((piece, index) => {
    const explanation = definitions.get(piece.toLocaleLowerCase('de-DE'));
    return explanation
      ? <abbr className="glossary-term" title={explanation} key={`${piece}-${index}`}>{piece}</abbr>
      : piece;
  })}</>;
}
